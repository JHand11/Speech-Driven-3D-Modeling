...
def predict_rotation_gradio(model, raw_im, delta_x, delta_y, pic_num, save_path = "", device="cuda", ddim_steps=75, scale=3.0):
    input_im_init = np.asarray(raw_im, dtype=np.float32) / 255.0
    input_im = transforms.ToTensor()(input_im_init).unsqueeze(0).to(device)
    input_im = input_im * 2 - 1

    sampler = DDIMSampler(model)
    x_samples_ddims = sample_model_batch(model, sampler, input_im, [delta_x], [delta_y], n_samples=1, ddim_steps=ddim_steps, scale=scale)

    x_sample = 255.0 * rearrange(x_samples_ddims[0].numpy(), 'c h w -> h w c')
    out_image = Image.fromarray(x_sample.astype(np.uint8))

    if save_path:
        out_image.save(os.path.join(save_path, '{}.png'.format(pic_num)))
...
def infer_shifted_images_for_folder(model, input_dir_path, save_path_stage2, delta_x, delta_y, device, pic_start, ddim_steps=75, scale=3.0):
    # Assume input_dir_path contains Stage 1 images to process
    # save_path_stage2 is where the shifted images will be saved
    
    stage1_image_paths = [os.path.join(input_dir_path, f) for f in os.listdir(input_dir_path) if f.endswith('.png')]
    sampler = DDIMSampler(model)
    image_counter = pic_start  # Start naming images from number 4

    for idx, stage1_image_path in enumerate(stage1_image_paths):
        raw_im = Image.open(stage1_image_path)
        input_im_init = np.asarray(raw_im, dtype=np.float32) / 255.0  # Normalize the image
        input_im = transforms.ToTensor()(input_im_init).unsqueeze(0).to(device)
        input_im = input_im * 2 - 1  # Scale to [-1, 1]

        # Generate one new image with specified shift
        x_samples_ddims_stage2 = sample_model_batch(model, sampler, input_im, [delta_x], [delta_y], n_samples=1, ddim_steps=150, scale=scale)

        # Save the generated image with naming from 4 onwards
        x_sample_stage2 = 255.0 * rearrange(x_samples_ddims_stage2[0].cpu().numpy(), 'c h w -> h w c')
        output_filename = f"{image_counter}.png"  # Constructs filename as "4.png", "5.png", etc.
        Image.fromarray(x_sample_stage2.astype(np.uint8)).save(os.path.join(save_path_stage2, output_filename))
